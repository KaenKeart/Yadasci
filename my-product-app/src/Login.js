...
export default function Login() {
  ...
  const getAuthenToken = async () => {
    const response = await fetch("http://localhost:8000/api/authen_request", {
      method: "POST",
      headers: {
  ...
  const getAccessToken = async (authToken) => {
    ...
    const response = await fetch("http://localhost:8000/api/access_request", {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
  ...
  const doLogin = async () => {
    ...
    localStorage.setItem("gender", data2.data.account_info.gender);
